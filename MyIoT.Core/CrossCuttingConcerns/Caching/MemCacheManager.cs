...
public class MemCacheManager : ICacheManager
{
    ...
    public MemCacheManager() => _memoryCache = ServiceHelper.ServiceProvider.GetService<IMemoryCache>();

    public void Add(string key, object data, int duaration)
    {
        _memoryCache.Set(key, data, TimeSpan.FromMinutes(duaration));
    }
